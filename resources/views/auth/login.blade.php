            ...
            <div class="w-full max-w-md">
                <div class="text-center mb-8">
                    ...
                    <p class="mt-2 text-center text-gray-200 text-sm">
                        Silakan masukkan name dan password untuk login
                    </p>
                </div>
                ...
                <form action="{{ route('login.post') }}" method="POST">
                    ...
                    <div class="grid gap-y-4">
                        <div>
                            <label for="name" class="block text-sm mb-2 text-yellow-500 dark:text-yellow-400">name</label>
                            <div class="relative">
                                <input type="name" id="name" name="name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="nama@name.com" required aria-describedby="name-error">
                                @error('name')
                                    <p class="mt-2 text-sm text-red-600 dark:text-red-500" id="name-error">{{ $message }}</p>
                                @enderror
                            </div>
                        </div>
                        <div>
